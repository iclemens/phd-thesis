...
source = re.sub("\\\\\"i", "&iuml;", source)
source = re.sub("\\\\\"e", "&euml;", source)
source = re.sub("\\\\\'e", "&eacute;", source)
...
def chapterHTMLGroup(match):
	namerefs[match.group(2)] = match.group(1)
	return "<h1 name=\"" + match.group(2) + "\">" + match.group(1) + "</h1>"
...
def resolveChapters(source):
	...
	source = re.sub("\\\\chapter{([^}]*)}\s*\\\\chaptermark{}\s*\\\\label{([^}]*)}", chapterHTMLGroup, source, flags = re.MULTILINE)
	source = re.sub("\\\\chapter{([^}]*)}", chapterHTML, source, flags = re.MULTILINE)
...
def hoofdstukRefHTML(match):
	if not match.group(1) in namerefs:
		return "<a href=\"#\">Undefined chapter label</a>"
	return "<a href=\"#" + match.group(1) + "\">Hoofdstuk " + namerefs[match.group(1)] + "</a>"
...
def resolveRefs(source):
	...
	source = re.sub("\\\\Hoofdstuk{([^}]*?)}", hoofdstukRefHTML, source, flags = re.MULTILINE)

	return source

source = resolveRefs(source)

def descriptionItemHTML(match):
	code = "<li>"
	code += "<b>" + match.group(1) + "</b>: " + match.group(2)
	code += "</li>"
	return code

def descriptionHTML(match):
	code = "<ul>"
	code += re.sub("\\\\item\[(.*?)\](.*)", descriptionItemHTML, match.group(1), flags = re.MULTILINE)
	code += "</ul>"
	return code

def resolveDescription(source):
	source = re.sub("\\\\begin{description}([\s\S]*?)\\\\end{description}", descriptionHTML, source, flags = re.MULTILINE)
	return source

source = resolveDescription(source)


def citationHTML(match):
	return "(<a href=\"#\">" + match.group(1) + "</a>)"

def resolveCitations(source):
	return re.sub("\\\\cite{([^}]*)}", citationHTML, source, flags = re.MULTILINE)
