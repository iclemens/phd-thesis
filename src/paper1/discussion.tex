...
in which sHS denotes the noise in the sensory signal, known to the observer, and $w_{HS}$ and $w_{HP}$ represent the relative weights of the sensory signal and the prior, respectively. Note that Equation \ref{p1:eqn13} is equivalent to Equation \ref{p1:eqn1} in \nameref{p1:sec:methods}. The variance of the posterior distribution in a single trial is given by the following: 
